-- Tüm tablolar için eksik RLS politikalarını ekle
...
-- (Zaten mevcut görünüyor ama kontrol edelim)
ALTER TABLE public.ayarlar ENABLE ROW LEVEL SECURITY;

CREATE POLICY IF NOT EXISTS "Allow public read access to settings" ON public.ayarlar
  FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "Allow admin update settings" ON public.ayarlar
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE user_id = auth.uid() AND role = 'admin'
...
CREATE POLICY IF NOT EXISTS "Allow admin insert settings" ON public.ayarlar
...
CREATE POLICY IF NOT EXISTS "Allow admin delete settings" ON public.ayarlar
  FOR DELETE USING (
    EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE user_id = auth.uid() AND role = 'admin'
